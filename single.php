...
get_header();?>
	...
	$vslide_title = get_post_meta( get_the_ID(), 'vertical_slide_title', true );
	$vslide_content = get_post_meta( get_the_ID(), 'vertical_slide_content', true );
	// Video Background
	$video_bkg = get_post_meta( get_the_ID(), 'video', true );
	$video_src = get_attached_media( $video_bkg );
	$video_url = wp_get_attachment_url( $video_bkg );
	// Full Size Image Background
	$bkg_img = get_post_meta( get_the_ID(), 'image', true );
	...
	<?php if ( get_post_meta ( get_the_ID(), 'change_slide_background', true )  == 'Video' ) : ?>
	...
	<?php if ( 'Select One' == get_post_meta( get_the_ID(), 'change_slide_background', true ) ) :?>
	<section>
<?php endif;?>
	...
	<?php // subtitle ?>
	<?php $subtitle = get_post_meta( get_the_ID(), 'subtitle', true );if( $subtitle ) :?>
		<h3 class="title"><?php echo $subtitle;?></h3>
	<?php endif ?>

	<?php // content ?>
	<?php $content_title_content = get_post_meta( get_the_ID(), 'slide_content_title_content', true );if( $content_title_content ) :?>
		<div class="content"><?php echo $content_title_content;?></div>
	<?php endif ?>

	<?php // content left column ?>
	<?php $content_left = get_post_meta( get_the_ID(), 'content_left_column_two_columns', true );if( $content_left ) :?>
		<div class="content-left"><?php echo $content_left;?></div>
	<?php endif ?>

	<?php // content right column ?>
	<?php $content_right = get_post_meta( get_the_ID(), 'content_right_column_two_columns', true );if( $content_right ) :?>
		<div class="content-right"><?php echo $content_right;?></div>
	<?php endif ?>

	<?php // content - content left image right ?>
	<?php $content_left_image_right = get_post_meta( get_the_ID(), 'content_content_image_right', true );if( $content_left_image_right ) :?>
		<div class="content-left"><?php echo $content_left_image_right;?></div>
	<?php endif ?>

	<?php // image - content left image right ?>
	<?php $image_right = get_post_meta( get_the_ID(), 'image_content_image_right', true );if( $image_right ) :
	...
	<?php $image = get_post_meta( get_the_ID(), 'image_title_image', true );if( $image ) :
