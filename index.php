...
<?php endif; ?>
	...
	<?php // subtitle ?>
	<?php $subtitle = get_post_meta( $slide->ID, 'subtitle', true );
	if ( $subtitle ) : ?>
		<h3 class="title"><?php echo $subtitle; ?></h3>
	<?php endif ?>

	<?php // content ?>
	<?php $content_title_content = get_post_meta( $slide->ID, 'slide_content_title_content', true );
	if ( $content_title_content ) : ?>
		<div class="content"><?php echo wpautop( $content_title_content ); ?></div>
	<?php endif ?>

	<?php // content left column ?>
	<?php $content_left = get_post_meta( $slide->ID, 'content_left_column_two_columns', true );
	if ( $content_left ) : ?>
		<div class="content-left"><?php echo wpautop( $content_left ); ?></div>
	<?php endif ?>

	<?php // content right column ?>
	<?php $content_right = get_post_meta( $slide->ID, 'content_right_column_two_columns', true );
	if ( $content_right ) : ?>
		<div class="content-right"><?php echo wpautop( $content_right ); ?></div>
	<?php endif ?>

	<?php // content - content left image right ?>
	<?php $content_left_image_right = get_post_meta( $slide->ID, 'content_content_image_right', true );
	...
	<?php $image_right = get_post_meta( $slide->ID, 'image_content_image_right', true );
	if ( $image_right ) :
	...
	<?php $image = get_post_meta( $slide->ID, 'image_title_image', true );
	if ( $image ) :
	...
	<?php // code ?>
	<?php $code = get_post_meta( $slide->ID, 'code', true );
	if ( $code ) : ?>
		<pre>
							<code data-trim>
								<?php echo $code; ?>
